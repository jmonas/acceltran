...
class ViTSelfAttention(nn.Module):
    ...
    def prune_weights(self):
        self.query.weight = dynatran_prune(self.query.weight, self.pruning_threshold, self.sparsity_file, parameter=True)
        self.key.weight = dynatran_prune(self.key.weight, self.pruning_threshold, self.sparsity_file, parameter=True)
        self.value.weight = dynatran_prune(self.value.weight, self.pruning_threshold, self.sparsity_file, parameter=True)
    ...
    def forward(self, hidden_states, head_mask=None, output_attentions=False):
        ...
        key_layer = dynatran_prune(key_layer, self.pruning_threshold, self.sparsity_file)
        value_layer = dynatran_prune(value_layer, self.pruning_threshold, self.sparsity_file)
        query_layer = dynatran_prune(query_layer, self.pruning_threshold, self.sparsity_file)


        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))

        attention_scores = dynatran_prune(attention_scores, self.pruning_threshold, self.sparsity_file)
        ...
        attention_probs = dynatran_prune(attention_probs, self.pruning_threshold, self.sparsity_file)
        ...
        context_layer = dynatran_prune(context_layer, self.pruning_threshold, self.sparsity_file)
...
class ViTSelfOutput(nn.Module):
    ...
    def prune_weights(self):
        self.dense.weight = dynatran_prune(self.dense.weight, self.pruning_threshold, self.sparsity_file, parameter=True)

    def forward(self, hidden_states, input_tensor):
        hidden_states = dynatran_prune(self.dense(hidden_states), self.pruning_threshold, self.sparsity_file)
...
class ViTAttention(nn.Module):
    ...
    def prune_weights(self):
        self.self.prune_weights()
        self.output.prune_weights()
...
class ViTLayer(nn.Module):
    ...
    def prune_weights(self):
        self.attention.prune_weights()
        self.intermediate.prune_weights()
        self.output.prune_weights()
...
class DTViTModel(ViTPreTrainedModel):
    ...
    def prune_weights(self):
        self.encoder.prune_weights()
        if self.pooler: self.pooler.prune_weights()
...
class ViTPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        pooled_output = dynatran_prune(self.dense(first_token_tensor), self.pruning_threshold, self.sparsity_file)
...
class DTViTForImageClassification(ViTPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.num_labels = config.num_labels
        self.vit = DTViTModel(config, add_pooling_layer=False)
