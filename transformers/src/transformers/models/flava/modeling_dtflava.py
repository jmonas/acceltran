...
class FlavaImageEmbeddings(nn.Module):
    ...
    def __init__(self, config: FlavaImageConfig, use_mask_token: bool = False) -> None:
        ...
        self.patch_embeddings = PatchEmbeddings(config = config)
...
class PatchEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        num_channels, embed_dim = config.num_channels, config.hidden_size
    ...
    def forward(self, pixel_values: torch.Tensor, interpolate_pos_encoding: bool = False) -> torch.Tensor:
        ...
        x = dynatran_prune(x, self.pruning_threshold, self.sparsity_file)
        return x
...
class DTFlavaModel(FlavaPreTrainedModel):
    config_class = FlavaConfig
